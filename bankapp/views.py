...
def login(request):
    if request.method =="POST":
        ...
        # print(user_object)
        # print(card_number)
        # print(account_person)
        # print(recc_account_details)
        ...
        # print(full_name,account_number,account_type,card,"\n",logined_user_account_status)

        #creating session storage
        request.session['user_details']=[full_name,account_number,account_type]
        request.session['full_name']=full_name
        request.session['card']=[card["card_number"],card["cvv_number"]]
        # request.session['card_number']=card["card_number"]
        # request.session['cvv_number']=card["cvv_number"]
        # request.session['expiry_date']=card["expiry_date"]
...
def dashboard(request):
    ...
    context={
        'full_name':request.session.get('user_details')[0],
        'account_number':request.session.get('user_details')[1],
        'account_type':request.session.get('user_details')[2],

        'card_number':request.session.get('card')[0],
        'cvv_number':request.session.get('card')[1],
        # 'expiry_date':request.session.get('card').expiry_date,
